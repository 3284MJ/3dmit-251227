...
window.addEventListener('pointermove', (e) => { 
    ...
    if (isRingMenuOpen) {
        updateRingMenuSelection(e.clientX, e.clientY);
    }
});
...
function animate() {
    ...
    if (model) {
        ...
        if (!isOpening && !isDragging && !isRingMenuOpen) controls.target.lerp(model.position.clone().setY(0.5), 0.1);
    }
